...
since the supporting runtime is designed to explore parallelism implicitly.  In
our experiments we measured a 20\% overhead for N-Queens, a program that needs
to reference count many lists during run time. Fortunately, if the programmer
desires to use the parallel facilities of LM, she will be able to run programs
faster than C using between 2 or 4 threads.
